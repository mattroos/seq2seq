...
## Make separate arrays for all train and validation sets (as used under training)
dataTrain = dataReal[ixTrainReal]
dataVal = dataReal[ixValReal]
labelsTrain = labelsReal[ixTrainReal]
labelsVal = labelsReal[ixValReal]
inputLengthTrain = inputLengthReal[ixTrainReal]
inputLengthVal = inputLengthReal[ixValReal]
labelLengthTrain = labelLengthReal[ixTrainReal]
labelLengthVal = labelLengthReal[ixValReal]
lineStringsTrain = lineStringsRealTrain
lineStringsVal = lineStringsRealVal
...
# ## Build the model.... (JUST LOAD THE MODEL INSTEAD?)
# nKernSize = 3
# # nFilters = (8, 16, 32)
# nFilters = (8, 16)
...
# bUseSavedModel = True
# basename = model_dotnum_epoch100_seed6357_usdot_synth_ctc   #512-node GRUs
# basename = model_dotnum_epoch200_seed8413_usdot_synth_ctc   #256-node GRUs

# if bUseSavedModel:
#     model = model_from_json(open(basename + '_arch.json').read())
#     model.load_weights(basename + '_weights.h5')
...
# input_data = layers.Input(name='the_input', shape=dataVal.shape[1:], dtype='float32')
# y_pred = layers.Activation('softmax', name='softmax')(outer)
# model_pred = Model(inputs=input_data, outputs=y_pred)

# basename = 'model_dotnum_epoch100_seed6357_usdot_synth_ctc'   #512-node GRUs
basename = 'model_dotnum_epoch200_seed8413_usdot_synth_ctc'   #256-node GRUs
model_pred = model_from_json(open(basename + '_arch.json').read())
model_pred.load_weights(basename + '_weights.h5')


nSamples = len(dataVal)
# # Create dummy labels and other model input, if they don't exist.
# labelsVal = 11*np.ones((nSamples,17))
# inputLengthVal = 62*np.ones((nSamples,1))
# labelLengthVal = np.ones((nSamples,1))
# lineStringsVal = None
...
## Do AD HOC decoding of GRU output to get digit string.
#  Should do this with more formal method, e.g., HMM model.
#
# Decoding states:
# 11 = 11 (nothing/empty)
# 10 = 10 (char break)
...
for iSample in range(nSamples):
    ...
    for iSlice in range(1,softmaxVal.shape[1]):
        if state==11:
            ...
            mx = softmaxVal[iSample,iSlice,digArgMax]
            if mx >= thDigit:
                state = 0
...
for iSample in range(nSamples):
    ...
    plt.subplot(2,1,1)
    plt.imshow(np.flip(np.transpose(dataVal[iSample,:,:,0],(1,0)), axis=0))
